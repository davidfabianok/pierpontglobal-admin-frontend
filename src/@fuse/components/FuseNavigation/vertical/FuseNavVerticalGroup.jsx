...
function FuseNavVerticalGroup({
  classes, item, nestedLevel, userRole, active,
}) {
  if (item.auth && (!item.auth.includes(userRole) || (userRole !== 'guest' && item.auth.length === 1 && item.auth.includes('guest')))) {
    return null;
  }

  const paddingValue = 40 + (nestedLevel * 16);
  const listItemPadding = nestedLevel > 0 ? `pl-${paddingValue > 80 ? 80 : paddingValue}` : 'pl-24';

  return (
    <React.Fragment>

      <ListSubheader disableSticky className={classNames(classes.item, listItemPadding, 'list-subheader flex items-center')}>
